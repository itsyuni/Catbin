...
if (isset($_GET['username'])) {
        if (DB::query('SELECT username FROM users WHERE username=:username', array(':username'=>$_GET['username']))) {
                ...
                $worker = DB::query('SELECT worker FROM users WHERE username=:username', array(':username'=>$_GET['username']))[0]['worker'];
...
<!-- /Yandex.Metrika counter -->
 ...
 <header class="header header-sticky header-transparent" >
    ...
    <?php if ($pageIsLive) {
        ?>
        <center><h1 style="color: aliceblue;"><?php echo $username; if ($verified) { ?> <img src="/assets/img/user_verf.png" width="32px" height="32px"> <?php } else if ($worker) { ?> <img src="/assets/img/catbinworker_verf.png" width="32px" height="32px"> <?php } ?></h1></center>
    <form action="@<?php echo $username; ?>" method="post">
    <br>
    <?php if($photourl) { ?>
        <center><img src="<?php echo $photourl; ?>" width="150px" height="150px"></center> <?php
    } else { ?>
        <center><img src="/nophoto.png" width="150px" height="150px"></center><br><br> <?php
    } ?>
    <center><h5><?php
    if ($about) {
        echo $about;
    } else { ?>
     Привет, я пользователь Catbin!
     <?php     
    }  ?></h5></center>
    <?php
        if ($userid != $followerid) { ?>
        <?php
                if ($isFollowing) { ?>
                        <br><br><center><input type="submit" class="btn btn-primary" name="unfollow" value="Отписаться"></center>
                <?php } else { ?>
                        <br><br><center><input type="submit" class="btn btn-primary" name="follow" value="Подписаться"></center>
                <?php }
        } else { ?>
                <br><br><center><a href="/edit/" class="btn btn-primary">Редактировать профиль</a></center> <?php
        }
        ?>
    </form>
    <?php } else {
            ?> <center><h1>Страница не найдена!</h1></center>
    <?php } ?>
