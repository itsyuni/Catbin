...
if (isset($_GET['username'])) {
        if (DB::query('SELECT username FROM users WHERE username=:username', array(':username'=>$_GET['username']))) {
                ...
                echo "<br><br><center><h3>Страница не найдена</h3></center>" ;
        }
}
...
function getUsername() {
        ...
        if(isset($_POST['btnsb'])) {
                $host = "95.181.157.246";
                        ...
                        $password = "chikiMOHO";
                        $dbname = "catbin_u";
                        $usernamet = $_POST['usernamett'];
                        $aboutt = $_POST['abouttt'];
                        $userid = isLoggedIn();
                        if(!empty($_POST['usernamett'])) {
                                if (strlen($usernamet) >= 5 && strlen($usernamet) <= 15) {
                                        if (!preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $usernamet)) {
                                                if (!DB::query('SELECT username FROM users WHERE username=:username', array(':username'=>$usernamet))) {
                                                        DB::query('UPDATE users SET username=:usernamea WHERE id=:userid', array(':userid'=>$userid, ':usernamea'=>$usernamet));
                                                        echo '<center><br><br><div class="alertlog"><div class="alert alert-success" role="alert">Никнейм успешно изменён!</div></div>';
                                                } else {
                                                        echo '<center><br><br><br><br><br><div class="alertlog"><div class="alert alert-success" role="alert">Такой никнейм уже занят!</div></div>';
                                                }
                                        } else {
                                                echo '<center><br><br><br><br><br><div class="alertlog"><div class="alert alert-danger" role="alert">Символы /[\'^£$%&*()}{@#~?><>,|=_+¬-]/ запрещены к использованию в никнейме!</div></div>';
                                        }
                                } else {
                                        echo '<center><br><br><br><br><br><div class="alertlog"><div class="alert alert-danger" role="alert">Никнейм недопустимой длины!</div></div>';
                                }           
                        }

                        if(!empty($_POST['abouttt'])) {
                                if (strlen($aboutt) >= 3 && strlen($aboutt) <= 500) {
                                        if (!preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $aboutt)) {
                                                DB::query('UPDATE users SET about=:abouta WHERE id=:userid', array(':userid'=>$userid, ':abouta'=>$aboutt));
                                                echo '<center><br><br><div class="alertlog"><div class="alert alert-success" role="alert">Описание успешно изменено!</div></div>';
                                        } else {
                                                echo '<center><br><br><br><br><br><div class="alertlog"><div class="alert alert-danger" role="alert">Символы /[\'^£$%&*()}{@#~?><>,|=_+¬-]/ запрещены к использованию в описании профиля!</div></div>';
                                        }
                                } else {
                                        echo '<center><br><br><br><br><br><div class="alertlog"><div class="alert alert-danger" role="alert">Описание недопустимой длины!</div></div>';
                                }
                        }
        }
...
<!-- /Yandex.Metrika counter -->
 ...
 <header class="header header-sticky header-transparent" >
    ...
    <br><br><br><br><br>
    <center><form action="/edit/profiledata/" class="form" method="POST">
        <br><br>
        <center><h3 class="form__title">Редактирование описания профиля</h3></center>
        <br>
        ...
        <div class="form__div">
            <input type="input" class="form__input" name="usernamett" id="usernamett">
            <label for="" class="form__label">Никнейм</label>
        </div>

        <div class="form__div">
            <input type="input" class="form__input" name="abouttt" id="abouttt">
            <label for="" class="form__label">Описание профиля</label>
        </div>

        <center><input type="submit" class="form__button" value="Обновить информацию профиля" name="btnsb"></input></center>
        <br><br><br>
    </form></center>